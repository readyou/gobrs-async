...
import com.heytap.ad.osync.test.task.*;
...
public class CaseFuture {
    ...
    public void future() {
        ...
        for (AsyncTask task : abList) {
            Future<Object> submit = executorService.submit(() -> task.execute(new Object(), null));
            futures.add(submit);
        }
        ...
        Future<Object> submit = executorService.submit(() -> gService.execute(new Object(), null));
        try {
            submit.get();
        } catch (InterruptedException e) {
            e.printStackTrace();
